    ...
    let decisions = current_indices
        ...
        .map(|index| {
            ...
            let is_inlier = planes
                .iter()
                .any(|plane| plane.distance(point) <= config.distance_threshold);
...
Options:
  ...
  --distance-threshold <m>      Point-to-plane inlier threshold
                                Default: 0.035
...
Stricter wall filtering:
  {program} input.pcd output_strict.pcd \
    --distance-threshold 0.025 \
    --max-planarity-ratio 0.10 \
